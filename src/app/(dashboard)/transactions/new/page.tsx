...
import {
  ...
  AlertCircle, ScanBarcode, Keyboard,
} from 'lucide-react'
...
export default function NewTransactionPage() {
  ...
  const [jobRef, setJobRef]                 = useState('')   // job reference for JobLedger
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (canChargeTo) {
      ...
      if (jobRef.trim())   body.job_order_id = jobRef.trim()  // stored as cross-system reference
    }
        ...
        <div className="p-6 max-w-xl">
          <Card>
            <CardContent className="py-16 flex flex-col items-center gap-3">
              ...
              <p className="text-lg font-semibold text-slate-900">Transaction recorded</p>
              <p className="text-sm text-slate-400">Redirecting to transactions…</p>
            </CardContent>
          </Card>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  {transactionTypes.map(t => (
                    ...
                    >
                      <input
                        ...
                        onChange={() => { setType(t.value); resetLocations(); setCustomer(''); setJobRef('') }}
                        className="sr-only"
                      />
              ...
              {/* Batch / expiry (purchase + FIFO/track_expiry) */}
              {type === 'purchase' && (product?.cost_method === 'fifo' || product?.track_expiry) && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
